                ...
                if not is_env_usage:
                    ...
                    violations.append(
                        RuleViolation(
                            ...
                            recommendation=(
                                f"Use environment variables for addresses:\n\n"
                                f"  // Instead of hardcoding:\n"
                                f"  address constant ADMIN = {address};\n\n"
                                f"  // Use environment variable:\n"
                                f"  address admin = vm.envAddress(\"ADMIN_ADDRESS\");\n\n"
                                f"  // Or with fallback:\n"
                                f"  address admin = vm.envOr(\n"
                                f"      \"ADMIN_ADDRESS\",\n"
                                f"      address(0x...)  // Fallback for testing\n"
                                f"  );\n\n"
                                f"Then set in .env file:\n"
                                f"  ADMIN_ADDRESS={address}\n\n"
                                f"Benefits:\n"
                                f"  - Easy to deploy to different networks\n"
                                f"  - Addresses in one place (.env)\n"
